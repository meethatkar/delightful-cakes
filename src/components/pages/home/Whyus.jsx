  ...
  return (
    <Section className="bg-surfaceAlt/10">
      <Container>
        ...
        <div ref={staggerRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 w-full mt-10">
          {features.map((feature, index) => {
            ...
            return (
              <div
                data-stagger
                key={index}
                className="group flex flex-col p-8 rounded-[2.5rem] bg-white border border-border/20 shadow-xs hover:shadow-[0_20px_40px_rgba(183,110,121,0.05)] hover:border-primary/10 transition-shadow transform-border duration-500 hover:-translate-y-1.5"
              >
                {/* Icon wrapper with soft color backgrounds */}
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 mx-auto transition-color duration-500 ${isPrimary
